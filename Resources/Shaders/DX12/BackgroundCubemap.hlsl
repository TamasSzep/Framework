...
ConstantBuffer<RenderPassCBType> RenderPass : register(b0);
...
VertexPos4Direction VSMain(VertexPos3TextCoordNormal input)
{
	...
	float4 worldPosition4 = mul(RenderPass.ViewProjectionMatrixInverse, float4(input.Position, 1.0f));
	output.Direction = worldPosition4.xyz / worldPosition4.w - RenderPass.CameraPosition;
