...
void EventManager::UnregisterEventListener(IEventListener* eventListener)
{
	for (unsigned i = 0; i < m_EventClassData.size(); i++)
	{
		auto& listeners = m_EventClassData[i].Listeners;
		listeners.erase(std::remove(listeners.begin(), listeners.end(), eventListener), listeners.end());
	}
}

void EventManager::UnregisterEventListenerThreadSafe(IEventListener* eventListener)
{
	m_Mutex.lock();
...
void EventManager::UnregisterEventListenersForEvent(unsigned eventClassId)
{
	for (unsigned i = 0; i < m_EventClassData.size(); i++)
	{
		auto& eventClassData = m_EventClassData[i];
		if (eventClassData.ClassId == eventClassId)
		{
			eventClassData.Listeners.clear();
		}
	}
}
