...
RigidTransformation Camera::ViewToTransformation(const glm::mat4& viewMatrix)
{
	glm::mat3 a(viewMatrix);
	auto aInv = glm::transpose(a);

	RigidTransformation transformation(Uninitialized);
	transformation.Orientation[0] = glm::normalize(-aInv[2]);
	transformation.Orientation[1] = glm::normalize(aInv[1]);
	transformation.Orientation[2] = glm::normalize(aInv[0]);
	transformation.Position = -aInv * glm::vec3(viewMatrix[3]);

	return transformation;
}

glm::mat4 Camera::TransformationToView(const ScaledTransformation& transformation)
{
	auto& position = transformation.Position;

	// Making the base orthonormal.
	auto direction = glm::normalize(transformation.A[0]);
	auto right = glm::normalize(glm::cross(direction, transformation.A[1]));
...
void Camera::UpdateViewMatrix()
{
	...
	{
		m_ViewMatrix = TransformationToView(m_SceneNode.GetWorldTransformation());
