...
VertexInputLayout VertexInputLayout::GetDummy()
{
	VertexInputLayout dummyIL;
	dummyIL.Elements = { { "Dummy", VertexElementType::Float, sizeof(float), 1 } };
	return dummyIL;
}
...
void Vertex_SOA_Data::AddBoneData(const BoneData& boneData)
{
	AddBoneData(BoneData(boneData));
}

void Vertex_SOA_Data::AddBoneData(BoneData&& boneData)
{
	// Limiting bone counts and normalizing weights.
	const unsigned influenceLimit = 4;
	if (boneData.MaxInfluenceCountPerVertex > influenceLimit)
	{
		auto influenceVector = boneData.GetInfluenceVector();
		BoneData::SortByWeights(influenceVector);
		BoneData::LimitMaximumBoneCountOnSorted(influenceVector, influenceLimit);
		BoneData::NormalizeWeights(influenceVector);
		boneData.SetInfluenceVector(influenceVector);
		RaiseWarning("Maximum bone count is reduced to 4 when adding bone data as vertex attributes.");
	}

	// Copying data in separate arrays.
	unsigned countVertices = boneData.GetCountVertices();
	Core::SimpleTypeVectorU<glm::uvec4> boneIndices(countVertices, glm::uvec4(0));
	Core::SimpleTypeVectorU<glm::vec4> boneWeights(countVertices, glm::vec4(0.0f));
	auto startIndices = boneData.VertexInfluenceStartIndices.GetArray();
	auto influences = boneData.Influences.GetArray();
	for (unsigned i = 0; i < countVertices; i++)
	{
		unsigned startIndex = startIndices[i];
		unsigned boneCount = boneData.GetCountBones(i);
		for (unsigned j = 0; j < boneCount; j++)
		{
			auto& influence = influences[startIndex + j];
			boneIndices[i][j] = influence.BoneIndex;
			boneWeights[i][j] = influence.Weight;
		}
	}

	AddBoneData(boneIndices, boneWeights);
}

void Vertex_SOA_Data::AddBoneData(const Core::SimpleTypeVectorU<glm::uvec4>& boneIndices,
	const Core::SimpleTypeVectorU<glm::vec4>& boneWeights)
{
	assert(boneIndices.GetSize() == boneWeights.GetSize());
	auto countVertices = boneIndices.GetSize();
	AddVertexElement(c_VertexBoneIndexVertexElement, boneIndices.GetArray(), countVertices);
	AddVertexElement(c_VertexBoneWeightVertexElement, boneWeights.GetArray(), countVertices);
}

bool Vertex_SOA_Data::RemoveBoneData()
{
	return RemoveVertexElement(c_VertexBoneIndexVertexElement)
		&& RemoveVertexElement(c_VertexBoneWeightVertexElement);
}
...
const glm::uvec4* Vertex_SOA_Data::GetVertexBoneIndices() const
{
	return GetVertexElements<glm::uvec4>(c_VertexBoneIndexVertexElement.Name.c_str());
}

const glm::vec4* Vertex_SOA_Data::GetVertexBoneWeights() const
{
	return GetVertexElements<glm::vec4>(c_VertexBoneWeightVertexElement.Name.c_str());
}
