...
{
	return{ pathHandler.GetPathFromResourcesDirectory("Shaders/" + fileName), ShaderSourceType::File,
...
bool ShaderProgramDescription::operator==(const ShaderProgramDescription& other) const
{
	NumericalEqualCompareBlock(Shaders.size());
	for (size_t i = 0; i < Shaders.size(); i++) StructureEqualCompareBlock(Shaders[i]);
	return true;
}
...
bool ShaderProgramDescription::operator<(const ShaderProgramDescription& other) const
{
	NumericalLessCompareBlock(Shaders.size());
	for (size_t i = 0; i < Shaders.size(); i++) StructureLessCompareBlock(Shaders[i]);
	return false;
}
...
void Shader::Initialize(const ShaderDescription& description)
{
	std::stringstream ss;
...
void ShaderProgram::Initialize(const ShaderProgramDescription& description, Shader** pShaders)
{
	...
	{
		glAttachShader(m_Handle, pShaders[i]->GetHandle());
	}
...
void ShaderProgram::SetInputLayout(const VertexInputLayout& inputLayout)
{
	SetInputLayout(inputLayout, Core::c_InvalidSizeU);
}

void ShaderProgram::SetInputLayout(const VertexInputLayout& inputLayout, unsigned vertexAttributeDivisor)
{
	int offset = 0;
...
void ShaderProgram::TrySetInputLayoutElements(const VertexInputLayout& inputLayout)
{
	TrySetInputLayoutElements(inputLayout, Core::c_InvalidSizeU);
}

void ShaderProgram::TrySetInputLayoutElements(const VertexInputLayout& inputLayout, unsigned vertexAttributeDivisor)
{
	int offset = 0;
	int stride = (int)inputLayout.GetVertexStride();
	auto& ilElements = inputLayout.Elements;
...
Shader* ShaderManager::GetShader(const ShaderDescription& description, bool forceRecompilation)
{
	...
	{
		shader = m_Shaders.Request();
		shader->Initialize(description);
...
ShaderProgram* ShaderManager::GetShaderProgram(const ShaderProgramDescription& description, bool forceRecompilation)
{
	auto it = m_ShaderProgramMap.find(description);

	ShaderProgram* program;
	if (forceRecompilation || it == m_ShaderProgramMap.end())
	{
		// Getting shaders.
		Shader* pShaders[(int)ShaderType::COUNT];
		for (size_t i = 0; i < description.Shaders.size(); i++)
		{
			pShaders[i] = GetShader(description.Shaders[i], forceRecompilation);
		}

		program = m_ShaderPrograms.Request();
		program->Initialize(description, pShaders);

		// This overwrites an existing mapping if the same shader description was encountered before and we use force recompilation now.
		m_ShaderProgramMap[description] = program;
	}
	else
	{
		program = it->second;
